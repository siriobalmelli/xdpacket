...
{
	return (field.len && field.mask);
}

NLC_PUBLIC __attribute__((pure)) uint64_t
				xdpk_hash(const void *start,
						uint16_t len,
						uint8_t mask,
						uint64_t *hashp);

NLC_PUBLIC __attribute__((pure)) uint64_t
				xdpk_field_hash(struct xdpk_field field,
						const void *pkt,
						size_t plen,
						uint64_t *hashp);
...
/*	xdpk_field_create()
 */
NLC_PUBLIC struct xdpk_field	xdpk_field_create(size_t offt,
						size_t len,
						uint8_t mask,
 ...
 */
NLC_PUBLIC char			*xdpk_field_print(struct xdpk_field field);

NLC_LOCAL __attribute__((const)) uint8_t
				xdpk_field_tailmask(uint16_t mlen);
NLC_LOCAL  uint8_t		xdpk_field_start_byte(struct xdpk_field field,
							const void *pkt,
							size_t plen);
